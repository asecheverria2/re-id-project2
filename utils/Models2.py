...
def image_classification(input_shape):
    ...
    model.add(keras.layers.Dense(128, activation='relu'))
    model.add(keras.layers.Dense(128, activation='relu'))
    model.add(keras.layers.Dense(751, activation='softmax'))
...
def silhouette_cnn_model(input_shape):
    ...
    cnn_model.add(keras.layers.Dense(128, activation='relu'))
    cnn_model.add(keras.layers.Dense(128, activation='relu'))
    cnn_model.add(keras.layers.Dense(751, activation='softmax'))
    loss_function = tf.keras.losses.CategoricalCrossentropy()
    optimization_function = tf.keras.optimizers.Adam(lr=0.0025, beta_1=0.9, beta_2=0.999, amsgrad=True)
...
def multi_input_model():
    ...
    # d = keras.layers.Dropout(0.8)(d)
    d = keras.layers.Dense(751, activation='softmax')(d)
